...
var textArea = document.querySelector('#one');
var currentDocument = new Document("Test Document", testContent);
var labels = ["Cats", "Dogs", "Animals", "Pets"];
var colors = ['yellow', 'blue', 'red', 'green'];
var currentCategory = 0;
// var yellowHighlights = currentDocument.getAnnotationListByCategory(0);
...
function handleHighlight(){
  ...
  var range = getRangeOfSelectedText();
  if(selectedInputRangeIsValid(range)){
    addAnnotationToCollection(range);
    renderTextareaHighlights();
...
//Actual draws the highlights on the textarea.
renderTextareaHighlights = function(){
  $('textarea').highlightWithinTextarea({
    highlight: [
      {
        highlight: currentDocument.getAnnotationListByCategory(0).getRanges(),
        className: colors[0]
      },
      {
        highlight: currentDocument.getAnnotationListByCategory(1).getRanges(),
        className: colors[1]
      },
      {
        highlight: currentDocument.getAnnotationListByCategory(2).getRanges(),
        className: colors[2]
      },
      {
        highlight: currentDocument.getAnnotationListByCategory(3).getRanges(),
        className: colors[3]
      }
    ]
  });
